        ...
        Object.defineProperty(this, 'collection', {
            get: function() {
                ...
                .then(async (objs) => {
                    for(let obj of objs)
                    {
                        let raw = await db(table_name)
                        .where(foreign_key, obj.primaryValue)
                        .andWhere(local_key, this.source.primaryValue)
                        .first();
    
                        obj.pivot = new Pivot(table_name, foreign_key, local_key, raw);
                    }
